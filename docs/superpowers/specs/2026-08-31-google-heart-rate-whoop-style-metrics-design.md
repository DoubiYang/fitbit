...
`daily-heart-rate-zones` 是阈值的唯一首选来源；不得以「历史最高心率」作为最大心率，也不得写死截图中的任意 bpm 数字。只有四个有序区间 `light`、`moderate`、`vigorous`、`peak` 都存在且边界不重叠时，才可为该日计算 Strain。Google 会按天调整阈值，因此历史日期必须保存并使用该日期的阈值，不能用今天的区间重算过去。
...
分钟覆盖采用固定规则：一个原始样本从其 `sampleTime` 延伸至下一样本，但最长只延伸 75 秒；跨度按本地分钟边界拆分。单分钟累积至少 30 秒有效覆盖才成为可用分钟。一个分钟有多个 context 时，取覆盖秒数最多者；相同覆盖秒数时优先 `ACTIVE`、再 `SEDENTARY`、最后未知。分钟只有在满足以下任一条件时计入 Strain：主导 context 为 `ACTIVE`，或它与已确认 `exercise` 区间重叠至少 30 秒。未分区或低于最低 Google 区间的活动分钟不产生剂量。

- 已过去的日期：当天 Google 阈值存在、原始心率总覆盖至少 480 分钟、当天最后一个有效样本距当地日结束不超过 180 分钟，且当地 `06:00–12:00`、`12:00–18:00`、`18:00–24:00` 三个时段各至少有 90 分钟覆盖、任一时段内无超过 240 分钟的连续覆盖缺口，才可标记为完整；否则是 `incomplete`，不输出完整日 Strain。DST 日按真实当地时间计算各时段。
- 当天：永远是进行中；Google 阈值存在、已累计至少 120 分钟原始心率覆盖且最新样本距当前时刻不超过 90 分钟时，才显示截至当前的 `provisional` Strain。
- 有完整日资格但活动归因分钟为零时，输出 0.0 Strain；不具备完整日资格时未知不是零。
...
用户首次设置自己的基础睡眠目标。对睡眠日期 `D`，系统每天计算动态睡眠需求：
...
1. **API 映射：** 用真实授权账户验证原始心率、每日区间与区间时长是否可用；记录 Fitbit Air 的实际返回形态，不把 API 文档假设写成数据事实。
...
6. **覆盖不足：** 对 30/75 秒分钟规则、480/120 分钟覆盖阈值、三段各 90 分钟及 240 分钟最大缺口、180/90 分钟最新样本阈值、DST 23/25 小时日写单元测试；缺区间、缺心率或同步失败时不输出“偏低”、不把未知算作零。
7. **可重放：** 对固定输入断言精确的 dose、0.1 Strain、动态睡眠需求和整数 Recovery；同一份聚合输入、相同 `metricVersion` 得到完全相同的结果，算法调整只产生新版本。
8. **安全：** Coach 和首页都不把 Recovery 当成高强度训练许可；低质量数据不生成个性化高强度建议，不诊断或承诺训练安全。
