...
- 使用现有 `lucide-react` 图标；不增加插画、照片、手写 SVG、emoji 或外部字体资源。

### 3.2 390px 首页布局

从上到下固定为以下阅读顺序：

1. **紧凑页头：** `节律`、`今日 · 9月2日` 和一个带可访问名称的设置链接。同步新鲜度作为小文本标记，而非独立卡。
...
在 `>= 768px`：内容最大宽度为约 68rem；页头与主内容使用完整容器宽度。恢复摘要占约 4 栏，全天负荷与时间线占约 8 栏；建议和餐食 CTA 位于下方同一阅读流。`<768px` 恢复与负荷依次纵向排列。任何尺寸不应回到固定 46rem 的窄单列，亦不可有横向滚动或被固定导航遮挡的内容。
...
7. 在同一 390×844 和 1440px 视口实际截图，和已选概念原图并列视觉 QA。390px 必须依次出现编辑式日期、恢复摘要、全天负荷数值/时间线、单条建议和一个深绿全宽餐食 CTA；重点检查中文排版、日期、分母、timeline 空档、按钮高度、固定导航与安全区。原图路径记录在本文档顶部，若本机资产被清理，必须由用户重新提供视觉源后再宣布视觉 QA 通过。
